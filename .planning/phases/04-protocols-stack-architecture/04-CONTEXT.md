...
- **D-04:** **Notes-only — no demo.** Protocols and stack-positioning are conversational/architectural, not numerical (unlike the Phase 1 EKF and Phase 2 power-flow demos). Time is better spent on IEC 61850 depth and the whiteboard architecture; Juan already has real K8s/MQTT/Grafana **production** experience to cite as the "I've built this" credibility.
...
- **D-09:** **No aggregate vocabulary-bridge table and no timed-drill / Q&A rehearsal loop here** — those are **Phase 6** (BRG-01..03, QNA-01..03). Per-note bridges + say-aloud tracks feed Phase 6 without duplicating it now.
