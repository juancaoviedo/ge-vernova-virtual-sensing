...
---
*Last updated: 2026-06-15 — Phase 7 (Integrated HTML Study Site) complete: a single offline-first HTML study site assembled into `/docs` (GitHub Pages-ready) via a re-runnable `docs/build_site.py` — card-grid hub + per-page sticky TOC tying together the AGMS architecture/patents research pages + an annotated draw.io diagram, all 16 Phase 1–5 notes converted to HTML with offline MathJax math, and a demos page (EKF / DC-power-flow WLS / federated) with embedded key-function code + inline results. noindex/robots.txt/.nojekyll for unlisted sharing, PUBLISH.md deploy guide, build-time validate_links() gate (24 pages, 0 broken links); offline math + navigation human-verified. Phases 0, 1, 2, and 6 remain not-started (Phase 7 was built from the already-completed Phase 1–5 note material).*
