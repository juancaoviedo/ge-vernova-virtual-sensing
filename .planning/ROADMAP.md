...
- [x] 08.1-03-PLAN.md — Auto-provisioned Grafana dashboard ieee33-fault-event.json (SPEC-8 minimums + 4 D-13 extras) over the fault_event bucket
- [x] 08.1-04-PLAN.md — README runbook (uv run fault-sim) + qualitative article-comparison note (SPEC-10)
...
- [x] 09-03-PLAN.md — measure.py data path: source switch, P_inj derivation, sensor selection, energised gate, snapshot writes
...
**Wave 2** *(blocked on Wave 1 completion)*
- [x] 10-03-PLAN.md — Estimators behind one interface: AC-WLS (chi2/LNR + rank reporting) + EKF (Joseph) + sqrt-UKF + FASE predictor/persistence foil
...
- [x] 10-07-PLAN.md — D-11 foundational: state = 64 = 2×32 (buses 1..32, bus 0 fixed reference); full-rank WLS G on well_observed (closes bug #3)
- [x] 10-08-PLAN.md — D-10: ac_model.injection_sensitivity (S=∂x/∂p over bus injections) + FASEPredictor.predict rewrite consuming Δp/Cov(ε) (closes bug #2)
